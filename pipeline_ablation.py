...
from modules.llm_path_ranking import LLMPathRanker, CascadePathRanker
...
VALID_STRATEGIES = {"gnn_rank", "llm_rank", "gnn_then_llm", "llm_then_gnn"}


class DualKnowAblationPipeline:


    def __init__(self, ranking_strategy="gnn_rank",
                 model_path=None, encoder_path=None, data_dir=None,
                 cascade_factor=2):
        assert ranking_strategy in VALID_STRATEGIES, \
            f"ranking_strategy 必须是 {VALID_STRATEGIES} 之一，收到: {ranking_strategy}"
    ...
    def initialize(self):
        ...
        self.gnn_ranker = PathRanker(encoder=self.encoder)
        self.llm_ranker = LLMPathRanker(llm=self.llm, encoder=self.encoder)
        self.cascade_ranker = CascadePathRanker(
            gnn_ranker=self.gnn_ranker,
        ...
        print(f"[Init] 排序策略: {self.ranking_strategy}")

    def _patch_tokenizer_pad(self):

        import inspect
        patched_count = 0

        def _do_patch(tokenizer, label=""):
            ...
            if getattr(original_pad, '_is_patched', False):
                return
            try:
                sig = inspect.signature(original_pad)
                if 'padding_side' not in sig.parameters:
                    def patched_pad(*args, _orig=original_pad, **kwargs):
                        kwargs.pop('padding_side', None)
                        return _orig(*args, **kwargs)
                    patched_pad._is_patched = True
                    tokenizer._pad = patched_pad
                    patched_count += 1
            except (ValueError, TypeError):
                pass

        _do_patch(getattr(self.llm, '_tokenizer', None), "wrapper")


        engine = getattr(self.llm, '_llm', None)
        if engine is not None:
            ...
            for attr_chain in [
                ...
                for attr in attr_chain:
                    ...
                    if obj is None:
                        break
                if obj is not None and hasattr(obj, '_pad'):
                    _do_patch(obj, f"vllm.{'.'.join(attr_chain)}")

        if patched_count > 0:
            print(f"[Init] 已修复 ChatGLM4Tokenizer padding_side 兼容性问题 (patched {patched_count} 处)")

    def _select_paths(self, paths, node_features, query_vector, subgraph,
                       entities, question, seeds):


        if self.ranking_strategy == "gnn_rank":

            return self.gnn_ranker.select_topk_paths(
                paths, node_features, query_vector, subgraph, entities,
                seed_ids=seeds, question=question
            )


        try:
            if self.ranking_strategy == "llm_rank":

                return self.llm_ranker.select_topk_paths(
                    paths, question, subgraph, entities
                )

            elif self.ranking_strategy == "gnn_then_llm":

                return self.cascade_ranker.select_topk_gnn_then_llm(
                    paths, node_features, query_vector,
                    subgraph, entities, question,
                    seed_ids=seeds
                )

            elif self.ranking_strategy == "llm_then_gnn":

                return self.cascade_ranker.select_topk_llm_then_gnn(
                    paths, node_features, query_vector,
                    subgraph, entities, question,
                    seed_ids=seeds
                )
            ...
            print(f"  [Fallback] {self.ranking_strategy} LLM打分失败 ({e})，回退到 GNN 排序")
            return self.gnn_ranker.select_topk_paths(
                paths, node_features, query_vector, subgraph, entities,
                seed_ids=seeds, question=question
            )

        raise ValueError(f"未知策略: {self.ranking_strategy}")

    def run(self, question: str, verbose=True) -> Dict:
        assert self._initialized, "请先 initialize()"
        r = {"question": question, "ranking_strategy": self.ranking_strategy}
        T = {}
        ...
        if verbose:
            print(f"\n[Q] {question}")
            print(f"[Strategy] {self.ranking_strategy}")
        ...
        t0 = time.perf_counter()
        paths = self.gnn_ranker.enumerate_paths(sg, seeds)

        if paths and nf:
            tp, si = self._select_paths(
                paths, nf, fq, sg, self.entities, question, seeds
            )
        else:
            tp, si = [], []
        ...
        if tp:
            try:
                r["top_paths_readable"] = self.gnn_ranker.paths_to_readable(tp, sg, self.entities)
            except Exception as e:
                r["top_paths_readable"] = [f"[error] {e}"]
            if hasattr(self.gnn_ranker, "paths_to_readable_struct"):
                try:
                    r["top_paths_struct"] = self.gnn_ranker.paths_to_readable_struct(tp, sg, self.entities)
                except Exception as e:
        ...
        T["path_ranking"] = time.perf_counter() - t0
    ...
    def batch_run(self, questions, save_path=None, verbose=False):
        ...
        for i, q in enumerate(questions):
            print(f"[{self.ranking_strategy}] [{i+1}/{len(questions)}] {q[:50]}...")
            try:
